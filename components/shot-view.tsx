...
export default function ShotView(props: IShotProps) {
  ...
  const {
    execTime: stillExecTime,
    setPending: setStillPending,
  ...
  const debounceHandlePromptNavigation = useDebouncedCallback(
  ...
  const navigatePrompts = (direction: 'prev' | 'next') => {
    ...
    const newIndex =
      direction === 'prev' ? currentPromptIndex - 1 : currentPromptIndex + 1
  ...
  const debounceHandleStillNavigation = useDebouncedCallback(
    handleStillNavigation,
    1000
  )
  ...
  const navigateStills = (direction: 'prev' | 'next') => {
    ...
    setIsVideoLoading(true)
    const newIndex =
      direction === 'prev' ? currentStillIndex - 1 : currentStillIndex + 1
  ...
  const debounceHandleVideoNavigation = useDebouncedCallback(
  ...
  const navigateVideos = (direction: 'prev' | 'next') => {
    const newIndex =
      direction === 'prev' ? currentVideoIndex - 1 : currentVideoIndex + 1
  ...
  const generateStill = async (prompt?: ShotPrompt) => {
    ...
    try {
      ...
      const response = await fetch('/api/gen_store_still', {
        ...
        body: JSON.stringify({
          ...
          prompt: encodeURIComponent(
            isEditable ? newPrompt?.prompt ?? '' : currentPrompt.prompt ?? ''
          ),
          seq_num: isEditable
            ? 0
            : stills[stills.length - 1]
              ? stills[stills.length - 1].seq_num! + 1
              : 0
        })
      })
      ...
      if (isEditable) {
        ...
        setStills([...stills.filter(s => s.id !== fakeStill.id), still])
        setCurrentStillIndex(stills.length)
      }
    } catch (error) {
      console.log(error)
    ...
    try {
      ...
      const response = await fetch('/api/gen_store_video', {
        ...
        body: JSON.stringify({
          ...
          seq_num: videos[videos.length - 1]
            ? videos[videos.length - 1].seq_num! + 1
            : 0
        })
      })
  ...
  return (
    <div className="flex flex-col flex-1">
            ...
            <div className="flex flex-col flex-1">
              {isEditable ? (
                <Textarea
                  ...
                  disabled={
                    !isEditable || isStillGenerating || isVideoGenerating
                  }
                />
              ) : (
              ...
              <div className="flex items-center mt-2">
                <Button
                  ...
                  onClick={() => navigatePrompts('prev')}
                  variant="ghost"
                  disabled={
                    !isPrevPromptAvailable ||
                    isStillGenerating ||
                    isVideoGenerating ||
                    isEditable
                  }
                ...
                <Button
                  className="rounded-full p-2"
                  onClick={() => navigatePrompts('next')}
                  variant="ghost"
                  disabled={
                    !isNextPromptAvailable ||
                    isStillGenerating ||
                    isVideoGenerating ||
                    isEditable
                  }
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={toggleEdit}
                  disabled={isStillGenerating || isVideoGenerating}
                >
                  <IconPencil />
                </Button>
              </div>
      ...
      <div>
        <div
          className="flex flex-col justify-center items-center border rounded-lg overflow-hidden min-h-[157px] relative"
          style={{ aspectRatio: 16 / 9 }}
        >
          {stillPending && currentStillIndex + 1 === stills?.length && (
            <div className="absolute w-full h-full top-0 left-0 bg-white dark:bg-neutral-950 opacity-100 flex items-center justify-center">
                ...
                <>
                  {isEditable ? (
                    ...
                    <ExpandableMedia
                      ...
                      close={() => setExpand(-1)}
                      open={() => setExpand(currentStillIndex)}
                    >
                      <img
                        className="cursor-pointer"
                        src={stills[currentStillIndex].still_url ?? ''}
        ...
        <div className="flex flex-wrap mt-2 gap-x-2 justify-between">
          {(stills?.length ?? 0) > 0 && !isEditable && (
            <div className="flex items-center space-x-2">
              <span className="w-12">Still: </span>
              <Button
                className="rounded-full p-2"
                variant="ghost"
                onClick={() => navigateStills('prev')}
                disabled={!isPrevStillAvailable || isVideoGenerating}
              >
                <IconChevronLeft />
              </Button>
              <span className="mx-2">
              ...
              <Button
                className="rounded-full p-2"
                variant="ghost"
                onClick={() => navigateStills('next')}
                disabled={!isNextStillAvailable || isVideoGenerating}
              >
                <IconChevronRight />
              </Button>
              {stills && stills[currentStillIndex] && (
